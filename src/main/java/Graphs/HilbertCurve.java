...
public class HilbertCurve {
	...
	public HilbertCurve() {
		...
		SequenceNucleotide sequence_DNA = (SequenceNucleotide) sequence_DNAs.get(0);
		...
		for (int start = 0; start < 3; start++) {
			for (int index = start; index < dna.length(); index++) {
				...
				if (mod == 2) {
					aminoAcid = AminoAcids.aminoAcidList[DigestionThread_DNA.indexForCodonArray(codon, forwards)];
					if (aminoAcid == 'M') {
						inORF = true;
					}
					if (aminoAcid == '.') {
