...
public class MatchContainer implements Comparable<MatchContainer> {
    ...
    public MatchContainer(Match match) {
        ...
        if (validPeptideFile) {
            ...
            if (!isTrue) {
                trueMatch = Properties.matchConstructor.createMatch(match.getSpectrumMatches(), new Peptide(this.correctAcidSequence));
            } else {
                trueMatch = match;
            }
    ...
    public String getCorrectAcidSequence() {
        return correctAcidSequence;
    }
