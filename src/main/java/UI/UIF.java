...
public class UIF {
    ...
    public static void text(int gridX, int gridY, int gridWidth, String label) {
        drawText(gridX * UIC.gridX, gridY * UIC.gridY, gridWidth * UIC.gridX, label);
    }

    public static void drawText(int xLoc, int yLoc, int pixelWidth, String label) {
        drawText(xLoc, xLoc, pixelWidth, label, UIC.ALIGN_LEFT);
    }

    public static void drawText(int xLoc, int yLoc, int pixelWidth, String label, int align) {

        int maxWidth = pixelWidth - UIC.gridXHalf;
        ...
        if (align == UIC.ALIGN_LEFT) {
            context.text(printed, xLoc + UIC.gridXFourth, yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
        if (align == UIC.ALIGN_CENTER) {
            context.text(printed, xLoc + (pixelWidth - labelWidth) / 2, yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
        if (align == UIC.ALIGN_RIGHT) {
            context.text(printed, xLoc + pixelWidth - (labelWidth + UIC.gridXFourth), yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
    ...
    public static void drawTextGraphics(int xLoc, int yLoc, int pixelWidth, String label, int align, PGraphics graphics) {
        ...
        //draw text based on how it is aligned
        if (align == UIC.ALIGN_LEFT) {
            graphics.text(printed, xLoc + UIC.gridXFourth, yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
        if (align == UIC.ALIGN_CENTER) {
            graphics.text(printed, xLoc + (pixelWidth - labelWidth) / 2, yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
        if (align == UIC.ALIGN_RIGHT) {
            graphics.text(printed, xLoc + pixelWidth - (labelWidth + UIC.gridXFourth), yLoc + UIC.gridYHalf + UIC.fontSizeThird);
        }
