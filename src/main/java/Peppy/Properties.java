...
//		leftIonDifference = 1.0;  //a, b, c ion
    ...
    public static void setProperty(String fieldName, String value) throws NoSuchFieldException, IllegalAccessException {
        ...
        if (field.getType() == Character.TYPE) {
            field.set(theClass, value.charAt(0));
            return;
        }
        if (field.getType() == Short.TYPE) {
            field.set(theClass, Short.parseShort(value));
            return;
        }
        if (field.getType() == Integer.TYPE) {
            field.set(theClass, Integer.parseInt(value));
            return;
        }
        if (field.getType() == Long.TYPE) {
            field.set(theClass, Long.parseLong(value));
            return;
        }
        if (field.getType() == Float.TYPE) {
            field.set(theClass, Float.parseFloat(value));
            return;
        }
        if (field.getType() == Double.TYPE) {
            field.set(theClass, Double.parseDouble(value));
            return;
        }
        if (field.getType() == Byte.TYPE) {
            field.set(theClass, Byte.parseByte(value));
            return;
        }
        if (field.getType() == Boolean.TYPE) {
            field.set(theClass, Boolean.parseBoolean(value));
            return;
        }
        if (field.getType() == File.class) {
            field.set(theClass, new File(value));
            return;
        }
