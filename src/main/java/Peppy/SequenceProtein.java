...
public class SequenceProtein extends Sequence {
    ...
    public ArrayList<Peptide> extractMorePeptides(boolean isReverse) {
        ...
        if (peptides.size() == 0)
            return null; //TODO fix.  there is the (small) possibility that this may return null and there are still proteins left to digest.
        return peptides;
    }
    ...
    public ArrayList<Protein> getProteinsFromDatabase(boolean isReverse, boolean limitAmount) {
        ...
        if (sequenceFile.getName().toLowerCase().endsWith("dat"))
            return getProteinsFromUniprotDAT(isReverse, limitAmount);
        return null;
    }
