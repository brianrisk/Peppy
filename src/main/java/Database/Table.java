        ...
        if (existingMatches == null) {
            ...
            if (keepOnlyTheBest) {
                ...
                if (bestColumn.getType().equals(Integer.class)) {
                    if (row.getInt(bestColumn.getName()) > existingMatches.get(0).getInt(bestColumn.getName()))
                        isBetter = true;
                    if (row.getInt(bestColumn.getName()) == existingMatches.get(0).getInt(bestColumn.getName()))
                        isEqual = true;
                }
                ...
                if (bestColumn.getType().equals(Double.class)) {
                    if (row.getDouble(bestColumn.getName()) > existingMatches.get(0).getDouble(bestColumn.getName()))
                        isBetter = true;
                    if (row.getDouble(bestColumn.getName()) == existingMatches.get(0).getDouble(bestColumn.getName()))
                        isEqual = true;
                }
