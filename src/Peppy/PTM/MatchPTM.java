 ...
 *
 */
public class MatchPTM implements Comparable<MatchPTM>, HasEValue{
	...
	public MatchPTM(String spectrumString, String peptideString) {
	...
	public MatchPTM(Spectrum spectrum, Peptide peptide, Sequence sequence) {
...
//		U.p(totalMatchingIntensityProbability);
		...
		impValue =  peakMatchProbability  * intensityProbability * totalMatchingIntensityProbability;
		score = Math.log(1 / impValue);
	...
	public int compareTo(MatchPTM matchPTM) {
			if (sortParameter == SORT_BY_SCORE) {
				//want to sort from greatest to least
				if (score > matchPTM.getScore()) return -1;
				if (score < matchPTM.getScore()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_SCORE_RATIO) {
				//want to sort from greatest to least
				if (scoreRatio > matchPTM.getScoreRatio()) return -1;
				if (scoreRatio < matchPTM.getScoreRatio()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_LOCUS) {
				if (sequence.getId() < matchPTM.getSequence().getId()) return -1;
				if (sequence.getId() > matchPTM.getSequence().getId()) return  1;
				//in case sequences equal, compare index
				if(peptide.getStartIndex() < matchPTM.getPeptide().getStartIndex()) return -1;
				if(peptide.getStartIndex() > matchPTM.getPeptide().getStartIndex()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_E_VALUE) {
				if (eValue < matchPTM.getEValue()) return -1;
				if (eValue > matchPTM.getEValue()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_P_VALUE) {
				if (pValue < matchPTM.getPValue()) return -1;
				if (pValue > matchPTM.getPValue()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_IMP_VALUE) {
				if (impValue < matchPTM.getImpValue()) return -1;
				if (impValue > matchPTM.getImpValue()) return  1;
				return 0;
			} else
			if (sortParameter == SORT_BY_SPECTRUM_ID_THEN_SCORE) {
				if (spectrum.getId() < matchPTM.getSpectrum().getId()) return -1;
				if (spectrum.getId() > matchPTM.getSpectrum().getId()) return  1;
			...
			if (sortParameter == SORT_BY_SPECTRUM_ID_THEN_PEPTIDE) {
				...
				return peptide.getAcidSequence().compareTo(matchPTM.getPeptide().getAcidSequence());
			} else	
			if (sortParameter == SORT_BY_RANK_THEN_E_VALUE) {
				if (rank < matchPTM.getRank()) return -1;
				if (rank > matchPTM.getRank()) return  1;
				if (eValue < matchPTM.getEValue()) return -1;
				if (eValue > matchPTM.getEValue()) return  1;
				return 0;
			...
			if (sortParameter == SORT_BY_SPECTRUM_PEPTIDE_MASS_DIFFERENCE) {
				...
				double theirDifference = matchPTM.getSpectrum().getPrecursorMass() - matchPTM.getPeptide().getMass();
	...
	public void setImpValue(double impValue) {
	...
	public boolean equals(MatchPTM matchPTM) {
		if (getScore() == matchPTM.getScore()) {
			if (peptide.equals(matchPTM.getPeptide())) return true;
		}
		return false;
	}
	...
	public static void setSortParameter(int sortParameter) {
		MatchPTM.sortParameter = sortParameter;
	}
